...
//
// THE ADVANCED CI TOUCHES
//

// The matrix covers the two things that actually differ - the toolchain
// version and the operating system - and stays small, because every cell is a
// job that costs minutes and a result someone has to read.
func TestTestMatrixCoversVersionsAndPlatforms(t *testing.T) {
	job := loadWorkflow(t, "ci.yml").Jobs["test"]

	versions, ok := job.Strategy.Matrix["go"].([]any)
	if !ok || len(versions) < 2 {
		t.Fatalf("the matrix covers %v Go versions, want at least two", job.Strategy.Matrix["go"])
	}

	if _, found := job.Strategy.Matrix["os"]; !found {
		t.Error("the matrix does not vary the operating system")
	}

	// One failing cell must not cancel the others: knowing that ONLY macOS
	// fails is most of the diagnosis.
	if job.Strategy.FailFast == nil || *job.Strategy.FailFast {
		t.Error("fail-fast is not disabled; one failing cell would hide the others")
	}
}

// A failing smoke test's log is the whole diagnosis; a passing one's is
// uninteresting. if: always() is what makes the failing case available.
func TestArtefactsAreKept(t *testing.T) {
	parsed := loadWorkflow(t, "ci.yml")
	...
	for _, job := range parsed.Jobs {
		for _, step := range job.Steps {
			...
			if retention, found := step.With["retention-days"]; !found {
				t.Errorf("artefact %q has no retention-days: storage nobody is paying attention to", name)
			} else if days, ok := retention.(int); ok && days > 30 {
				t.Errorf("artefact %q is kept for %d days", name, days)
			}
		}
	}

	for _, want := range []string{"coverage", "smoke-log"} {
		if !uploads[want] {
			t.Errorf("no %q artefact is uploaded", want)
		}
	}
}

// The smoke job runs the REAL binary: it would catch a broken migration or a
// config default that only fails at startup, which no unit test can.
func TestSmokeJobDrivesTheRealBinary(t *testing.T) {
	job, found := loadWorkflow(t, "ci.yml").Jobs["smoke"]
	if !found {
		t.Fatal("ci.yml has no smoke job")
	}
	...
	for _, want := range []string{"go build", "readyz", "api/links", "redirect_url", "metrics", "SIGTERM"} {
		if !strings.Contains(commands, want) && !strings.Contains(commands, strings.ToLower(want)) {
			t.Errorf("the smoke job does not exercise %q", want)
		}
	}
}

// A single summary check is what branch protection requires, so adding a job
// later does not mean editing the protection rule and forgetting.
func TestSummaryJobWaitsForEverythingAndFailsLoudly(t *testing.T) {
	parsed := loadWorkflow(t, "ci.yml")

	summary, found := parsed.Jobs["ci-passed"]
	if !found {
		t.Fatal("ci.yml has no ci-passed summary job")
	}

	needs, ok := summary.Needs.([]any)
	if !ok {
		t.Fatalf("ci-passed needs = %v, want a list", summary.Needs)
	}

	// Every other job must be waited on, or the summary lies.
	for jobName := range parsed.Jobs {
		if jobName == "ci-passed" {
			continue
		}
		...
		for _, need := range needs {
			if name, ok := need.(string); ok && name == jobName {
				waited = true
			}
		}

		if !waited {
			t.Errorf("ci-passed does not wait for %q", jobName)
		}
	}

	if summary.If != "always()" {
		t.Errorf("ci-passed has if: %q, want always() - otherwise it is skipped when a job fails", summary.If)
	}
	...
	// A skipped job is not a passing job, and treating it as one is how a
	// required check silently stops checking anything.
	if !strings.Contains(commands, "failure") || !strings.Contains(commands, "exit 1") {
		t.Error("ci-passed does not fail when a dependency failed")
	}
}
