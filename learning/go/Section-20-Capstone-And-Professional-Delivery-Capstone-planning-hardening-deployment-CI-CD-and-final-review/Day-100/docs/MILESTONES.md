...
Each day lives in its own directory (`Day-100` … `Day-100`) and carries the
...
- [x] Advanced CI: a matrix over two Go versions and two operating systems,
      coverage and smoke-log artefacts, a smoke job driving the **real binary**,
      and one summary check for branch protection
- [x] Self-review with the Day 91 checklist — and three real lint findings
      fixed (two unchecked writes of the API key, one always-false comparison)
- [x] `docs/FINAL-REVIEW.md`: the requirements reviewed against what shipped
- [x] The gaps, with triggers (`docs/GAPS.md`)
- [x] A portfolio README a stranger can start from
- [x] Reflection and the next five things to learn
