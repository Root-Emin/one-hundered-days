...
func TestHashIsStable(t *testing.T) {
	// The two hashes are computed into variables first. staticcheck flags
	// f(x) != f(x) as an always-false comparison, and it is right about the
	// shape - so the test says what it means instead: hash the same input
	// twice, in two statements, and compare the results.
	key := "lk_example"

	first := auth.Hash(key)
	second := auth.Hash(key)
