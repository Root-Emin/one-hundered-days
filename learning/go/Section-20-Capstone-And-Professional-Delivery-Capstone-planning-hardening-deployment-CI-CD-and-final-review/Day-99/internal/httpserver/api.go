...
import (
	...
	"example.com/onehundredday/Section-20-Capstone-And-Professional-Delivery-Capstone-planning-hardening-deployment-CI-CD-and-final-review/Day-99/internal/service"
	"example.com/onehundredday/Section-20-Capstone-And-Professional-Delivery-Capstone-planning-hardening-deployment-CI-CD-and-final-review/Day-99/internal/store"
)

// RedirectObserver counts redirect outcomes for metrics.
type RedirectObserver interface {
	RecordRedirect(outcome string)
	RecordClickDropped()
}
...
// SetObserver attaches metrics. Without one the API still works, unobserved -
// which is what keeps the handler tests free of a registry.
func (a *API) SetObserver(observer RedirectObserver) {
	a.observer = observer
}

func (a *API) recordRedirect(outcome string) {
	if a.observer != nil {
		a.observer.RecordRedirect(outcome)
	}
}
...
func (a *API) stats(w http.ResponseWriter, r *http.Request) {
	...
	if raw := r.URL.Query().Get("days"); raw != "" {
		...
		if err != nil || parsed < 1 || parsed > 365 {
			a.writeProblem(w, r, http.StatusBadRequest, "invalid_request", "days must be between 1 and 365")
	...
	stats, err := a.service.Stats(r.Context(), owner, r.PathValue("code"), days)
	if err != nil {
...
func (a *API) redirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, domain.ErrGone):
			a.recordRedirect("gone")
		case errors.Is(err, domain.ErrNotFound):
			a.recordRedirect("not_found")
		default:
			a.recordRedirect("error")
		}
...
func (a *API) recordClick(link domain.Link, r *http.Request) {
	...
	if err := a.service.RecordClick(ctx, click); err != nil {
		if a.observer != nil {
			// Counted, so "clicks are being dropped" is a number on a
			// dashboard rather than a line nobody greps for.
			a.observer.RecordClickDropped()
		}
